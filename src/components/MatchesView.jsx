...
const MatchesView = ({ theme }) => {
  ...
  const overdueMatches = upcoming.filter(
    m => m.matchDate && m.matchDate < today && m.status === 'Scheduled'
  );
  const futureMatches = upcoming.filter(
    m => !m.matchDate || m.matchDate >= today || m.status === 'Live'
  );
  const scheduled = matches.filter(m => m.status === 'Scheduled');

  // Dynamically find the most recent week that has results
  const weekNumbers = matchResults.map(m => parseInt(m.week)).filter(n => !isNaN(n));
      ...
      <Box p="6" overflowY="auto" flex="1">
        {!CURRENT_SEASON_ACTIVE ? (
          <Center py="20" flexDirection="column" gap="4">
            <Box color={emlColors.accentOrange} opacity={0.7}>
              <Archive size={44} />
            </Box>
          ...
          <Center py="20">
            <Spinner size="xl" color={emlColors.accentOrange} />
          </Center>
        ) : (
                        ...
                        <Table.Cell textAlign="center">
                          <HStack justify="center" gap="2">
                            <Text
                              ...
                              fontWeight="800"
                              color={
                                match.team1Won ? emlColors.semantic.win : emlColors.semantic.loss
                              }
                            >
                              {match.team1Score}
                            </Text>
                            <Text fontSize="sm" color={emlColors.textMuted} fontWeight="700">
                              —
                            </Text>
                            <Text
                              fontSize="md"
                              fontWeight="800"
                              color={
                                match.team2Won ? emlColors.semantic.win : emlColors.semantic.loss
                              }
                            >
                              {match.team2Score}
                            </Text>
                          ...
                          {match.isForfeit && (
                            <Badge colorPalette="yellow" size="xs" mt="1">
                              FORFEIT
                            </Badge>
                          )}
                        </Table.Cell>
                      ...
                      <Table.Body>
                        {overdueMatches.map(match => {
                          ...
                          return (
                            <Table.Row
                              key={match.id}
                              bg={`${emlColors.semantic.loss}08`}
                              _hover={{ bg: `${emlColors.semantic.loss}14` }}
                            >
                              <Table.Cell>
                              ...
                              <Table.Cell textAlign="center">
                                <Badge
                                  bg={`${emlColors.semantic.loss}22`}
                                  color={emlColors.semantic.loss}
                                  border={`1px solid ${emlColors.semantic.loss}44`}
                                  px="2"
                                  rounded="md"
                                  fontSize="xs"
                                  fontWeight="700"
                          ...
                          <Table.Cell>
                            <VStack align="start" gap="0">
                              ...
                              <Text fontSize="xs" color={emlColors.textMuted}>
                                {match.matchDate?.toLocaleTimeString('en-US', {
                                  hour: '2-digit',
                                  minute: '2-digit',
                                })}
                              </Text>
