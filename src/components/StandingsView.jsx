...
const TierDot = ({ tier, size = 'sm' }) => {
  ...
  return (
    <Box as="span" color={color} fontSize={size === 'sm' ? '10px' : '12px'} lineHeight="1">
      ◆
    </Box>
  );
};
  ...
  return (
    ...
    >
      {logo ? (
        ...
        <Text fontSize="xs" fontWeight="900" color={color} lineHeight="1">
          {initials}
        </Text>
      )}
    </Box>
...
const StandingsView = ({ theme }) => {
  ...
  const archivedBody = !CURRENT_SEASON_ACTIVE && (
    <Center py="16" flexDirection="column" gap="4">
      <Box color={colors.accentOrange} opacity={0.7}>
        <Archive size={40} />
      </Box>
            ...
            <HStack gap="2" flexWrap="wrap">
              {TIER_FILTERS.map(f => {
                ...
                return (
                  ...
                  >
                    {f !== 'All' && (
                      <Box as="span" mr="1.5" color={accent} fontSize="9px">
                        ◆
                      </Box>
                    )}
                    {f}
            ...
            >
              ...
              {filtered.map((team, index) => {
                ...
                const rankColor = isTop3
                  ? getMedalColor(pos, colors.accentOrange)
                  : colors.textSubtle;
                ...
                return (
                  ...
                  >
                    ...
                    <Box display="flex" alignItems="center" gap="3" minW="0" overflow="hidden">
                      <TeamAvatar
                        name={team.team}
                        color={isInactive ? colors.textSubtle : tierColor}
                      />
                      <Box minW="0">
                        <Text
                          fontSize="sm"
                    ...
                    <Box display="flex" alignItems="center" justifyContent="center" gap="0.5">
                      <Text
                        ...
                        fontSize="sm"
                        fontWeight="700"
                        color={colors.semantic?.win || '#22c55e'}
                      >
                        {team.wins ?? 0}
                      </Text>
                      <Text as="span" fontSize="sm" fontWeight="400" color={colors.textSubtle}>
                        -
                      </Text>
                      <Text
                        as="span"
                        fontSize="sm"
                        fontWeight="700"
                        color={colors.semantic?.loss || '#ef4444'}
                      >
                        {team.losses ?? 0}
                      </Text>
                    </Box>
